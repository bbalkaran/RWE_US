...
Migraine_complications_text <- list(grep("status migrainosus", OHDSI$concept_name, value = T, ignore.case = T),
                                    ...
                                    # grep("serotonin syndrome", OHDSI$concept_name, value = T, ignore.case = T),
                                    grep("diabetes mellitus", OHDSI$concept_name, value = T, ignore.case = T),
                                    grep("epilepsy", OHDSI$concept_name, value = T, ignore.case = T),
                                    grep("malaise", OHDSI$concept_name, value = T, ignore.case = T),
                                    grep("diabetes mellitus", OHDSI$concept_name, value = T, ignore.case = T),
                                    grep("hypertension",OHDSI$concept_name, value = T, ignore.case = T))
...
Migraine_complcations_ICD <- list(
  ...
  grep("L30.2",OHDSI$concept_code, value = T, ignore.case = T),
  grep("I49.9",OHDSI$concept_code, value = T, ignore.case = T),
  grep("K21.9",OHDSI$concept_code, value = T, ignore.case = T),
  grep("E78.0",OHDSI$concept_code, value = T, ignore.case = T),
  grep("I10",OHDSI$concept_code, value = T, ignore.case = T),
  grep("N92.0",OHDSI$concept_code, value = T, ignore.case = T),
  grep("I21.9",OHDSI$concept_code, value = T, ignore.case = T),
  grep("E66.01",OHDSI$concept_code, value = T, ignore.case = T),
  grep("E66.09",OHDSI$concept_code, value = T, ignore.case = T),
  grep("M81.0",OHDSI$concept_code, value = T, ignore.case = T),
  grep("G47.9",OHDSI$concept_code, value = T, ignore.case = T))
...
Migraine_complications <- Migraine_complications %>% 
  select(-standard_concept, -valid_start_date, -valid_end_date, -invalid_reason)%>% arrange(concept_name)


write.xlsx(Migraine_complications, 
           file = "C:/Users/BalkaranB/Kantar/Arunajadai, Srikesh (KH) - RWE_US/Lilly_migraine_prevention_161103482_2/Potential_Migraine_Complications_ICDcodes.xlsx")
